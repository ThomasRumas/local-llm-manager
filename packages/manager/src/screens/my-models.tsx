...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'LIST_DOWN':
      return {
        ...state,
        selectedIdx: Math.min(action.max, state.selectedIdx + 1),
      };
    case 'ENTER_ACTIONS':
      return { ...state, subStep: 'actions', actionIdx: 0 };
    case 'ACTION_UP':
      return { ...state, actionIdx: Math.max(0, state.actionIdx - 1) };
    case 'ACTION_DOWN':
      return {
        ...state,
        actionIdx: Math.min(ACTION_LABELS.length - 1, state.actionIdx + 1),
      };
    case 'BACK_TO_LIST':
      return { ...state, subStep: 'list' };
    case 'DELETE_START':
    ...
    case 'DELETE_ERROR':
      return { ...state, deleteError: action.error, deleting: false };
  }
}
...
function handleListKey(
  key: Key,
  models: LocalModel[] | null | undefined,
  dispatch: React.Dispatch<Action>,
): void {
  if (key.upArrow) dispatch({ type: 'LIST_UP' });
  if (key.downArrow)
    dispatch({ type: 'LIST_DOWN', max: (models?.length ?? 1) - 1 });
  if (key.return && models && models.length > 0)
    dispatch({ type: 'ENTER_ACTIONS' });
}

function handleActionsKey(
  key: Key,
  actionIdx: number,
  handleAction: (idx: number) => void,
  dispatch: React.Dispatch<Action>,
): void {
...
export function MyModels({ onBack, onNavigate }: Readonly<MyModelsProps>) {
  ...
  const handleAction = useCallback(
    async (idx: number) => {
      ...
      switch (idx) {
        case 0: {
          // Launch — go directly to model-launch if config already saved
          const savedConfigs = configService.getModelConfigNames(
    ...
    <Box flexDirection="column" gap={1} height="100%">
      ...
      {!loading && models && (
        <Box flexDirection="row" gap={2} height="100%">
          ...
          >
            <Text bold dimColor>
              MY MODELS <Text color="gray">({modelsDir})</Text>
            </Text>
            {models.length === 0 ? (
          ...
          {state.subStep === 'actions' && selectedModel && (
            ...
            >
              <Text bold color="cyan">
                {selectedModel.filename.replace(/\.gguf$/, '').slice(0, 18)}
              </Text>
              ...
              <Box flexDirection="column" marginTop={1}>
                {state.deleting ? (
                  ...
                  ACTION_LABELS.map((label, i) => (
                    <Box key={label} gap={1}>
                      <Text color={i === state.actionIdx ? 'cyan' : 'gray'}>
                        {i === state.actionIdx ? '❯' : ' '}
                      </Text>
                      <Text
                        color={actionTextColor(i === state.actionIdx, label)}
                      >
                        {label}
                      </Text>
                    </Box>
