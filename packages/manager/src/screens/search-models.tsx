...
import type {
  ...
  GGUFFile,
  DownloadProgress,
} from '../modules/huggingface/huggingface.types.js';
...
function filterDisplayTags(tags: string[]): string[] {
  return tags
    .filter(
      (t) =>
        GGUF_TAG_KEYWORDS.some((k) => t.toLowerCase().includes(k)) ||
        t.length < 20,
    )
    .slice(0, 4);
}
...
export function SearchModels({ onBack }: SearchModelsProps) {
  ...
  useInput((input, key) => {
    ...
    if (step === 'results' && !loadingFiles) {
      ...
      if (key.downArrow)
        setSelectedResultIdx((p) => Math.min(results.length - 1, p + 1));
    ...
    if (step === 'files') {
      ...
      if (key.downArrow)
        setSelectedFileIdx((p) => Math.min(ggufFiles.length - 1, p + 1));
  ...
  return (
    <Box flexDirection="column" gap={1} height="100%">
      ...
      <Box
        borderStyle="round"
        borderColor={step === 'input' ? 'cyan' : 'gray'}
        paddingX={1}
      >
      ...
      {step === 'results' && (
        <Box flexDirection="column" flexGrow={1} overflow="hidden">
          ...
          {loadingFiles && (
            <StatusBadge status="loading" label="Loading GGUF files…" />
          )}
      ...
      {step === 'files' && (
        <Box flexDirection="column" flexGrow={1} overflow="hidden">
          <Text dimColor>
            Files in{' '}
            <Text color="cyan">{results[selectedResultIdx]?.repoId}</Text>
            <Text color="gray">
              {' '}
              ({ggufFiles.length} quantization
              {ggufFiles.length !== 1 ? 's' : ''})
            </Text>
          </Text>
          {ggufFiles.length === 0 && (
            <Text color="yellow">No GGUF files found.</Text>
          )}
          {/* Column headers */}
          {ggufFiles.length > 0 && (
